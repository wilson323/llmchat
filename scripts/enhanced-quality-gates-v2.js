 ...
 * 
 * 修复内容：
 * 1. 改进错误处理逻辑，区分工具执行错误和代码质量问题
 * 2. 优化分数计算，支持部分检查失败的情况
 * 3. 提供更详细的错误诊断和建议
 * 4. 增强各项检查的容错性
 * 5. 新增 --allow-partial-failure 选项支持部分失败
 */
  ...
  checkESLint() {
    ...
    if (!result.success) {
      ...
      return {
        ...
        errors: 0,
        warnings: 0,
        score: isToolError ? 50 : 0, // 工具错误给部分分数，代码错误给0分
        issues: [],
        error: result.error,
        errorType: isToolError ? 'TOOL_ERROR' : 'CODE_ERROR',
        suggestion: isToolError 
          ? 'Install ESLint or check configuration' 
          : 'Fix ESLint errors in code'
      };
    }
    ...
    try {
      ...
      return {
        ...
        suggestion: 'Check ESLint configuration and output format'
      };
    }
  }
  ...
  checkSecurity() {
    ...
    if (!result.success) {
      ...
      return {
        ...
        moderate: 0,
        low: 0,
        score: isToolError ? 50 : 0,
        error: result.error,
        errorType: isToolError ? 'TOOL_ERROR' : 'AUDIT_ERROR',
        suggestion: isToolError 
          ? 'Install pnpm or check package manager' 
          : 'Run security audit manually to investigate'
      };
    }
    ...
    try {
      ...
      return {
        ...
        errorType: 'PARSE_ERROR',
        suggestion: 'Check audit output format and try running audit manually'
      };
    }
  }
  ...
  checkTestCoverage() {
    ...
    if (!result.success) {
      ...
      return {
        ...
        score: isToolError ? 50 : (testResult.success ? 70 : 0),
        ...
        suggestion: isToolError 
          ? 'Install test dependencies and configure test runner'
    ...
    if (fs.existsSync(coverageSummaryPath)) {
      try {
        ...
        return {
          ...
          errorType: 'PARSE_ERROR',
          suggestion: 'Check coverage report format and configuration'
        };
      }
    }
  ...
  checkBuild() {
    ...
    if (!result.success) {
      ...
      return {
        ...
        buildTime,
        buildSize: 0,
        score: isToolError ? 30 : 0,
        error: result.error,
        errorType: isToolError ? 'TOOL_ERROR' : 'BUILD_ERROR',
        suggestion: isToolError 
          ? 'Install build dependencies and configure build tools'
...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`
Enhanced Quality Gates System - Fixed Version v2

Usage: node scripts/enhanced-quality-gates-v2.js [options]
...
Examples:
  node scripts/enhanced-quality-gates-v2.js                    # Full checks
  node scripts/enhanced-quality-gates-v2.js --pre-commit       # Quick pre-commit checks
  node scripts/enhanced-quality-gates-v2.js --pre-push         # Pre-push validation
  node scripts/enhanced-quality-gates-v2.js --ci-mode          # CI/CD mode
  node scripts/enhanced-quality-gates-v2.js --allow-partial-failure  # Allow partial failures
  node scripts/enhanced-quality-gates-v2.js --mode ci         # Explicit CI mode
`);
    process.exit(0);
  }
