...
describe('TokenService', () => {
  ...
  describe('createAccessToken', () => {
    it('应该成功创建访问令牌', async () => {
      ...
      const token = await tokenService.createAccessToken('user1', 'testuser', 'user');
  ...
  describe('refreshAccessToken', () => {
    it('应该成功刷新访问令牌', async () => {
      ...
      const result = await tokenService.refreshAccessToken('valid-refresh-token', 'user1');
    ...
    it('应该拒绝无效的刷新令牌', async () => {
      ...
      const result = await tokenService.refreshAccessToken('invalid-refresh-token', 'user1');
    ...
    it('应该在用户不存在时失败', async () => {
      ...
      const result = await tokenService.refreshAccessToken('valid-refresh-token', 'user999');
  ...
  describe('revokeToken', () => {
    it('应该成功撤销单个令牌', async () => {
      ...
      expect(mockRedis.del).toHaveBeenCalledWith(expect.stringContaining('access:user1:'));
    });
  });
