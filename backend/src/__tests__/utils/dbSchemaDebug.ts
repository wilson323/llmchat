...
import { testDbConfig } from './dbTestUtils';

export async function debugDatabaseSchema(): Promise<void> {
  ...
  try {
    ...
    console.log('\n=== 检查users表结构 ===');
    const usersTableExists = tablesResult.rows.some((row: any) => row.table_name === 'users');
    ...
    console.log('\n=== 检查chat_sessions表结构 ===');
    const sessionsTableExists = tablesResult.rows.some((row: any) => row.table_name === 'chat_sessions');
    ...
    console.log('\n=== 检查chat_messages表结构 ===');
    const messagesTableExists = tablesResult.rows.some((row: any) => row.table_name === 'chat_messages');
