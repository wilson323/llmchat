...
export function auditMiddleware(
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (resourceType) {
      auditContext.resourceType = resourceType;
    }
...
async function recordAudit(req: Request, res: Response, responseBody: unknown) {
  try {
    ...
    if (req.audit?.resourceId) {
      logParams.resourceId = req.audit.resourceId;
    }
    if (ipAddress) {
      logParams.ipAddress = ipAddress;
    }
    if (userAgent) {
      logParams.userAgent = userAgent;
    }
    if (errorMessage) {
      logParams.errorMessage = errorMessage;
    }
...
export async function logAudit(params: {
  ...
  // 只添加非 undefined 的属性
  if (req.user?.id) {
    logParams.userId = req.user.id;
  }
  if (req.user?.username) {
    logParams.username = req.user.username;
  }
  if (resourceType) {
    logParams.resourceType = resourceType;
  }
  if (resourceId) {
    logParams.resourceId = resourceId;
  }
  if (details) {
