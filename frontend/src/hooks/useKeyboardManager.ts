...
export const useKeyboardManager = ({
  ...
  // 检查快捷键冲突
  const checkConflict = (shortcut: KeyboardShortcut): KeyboardShortcut | null => {
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    ...
    const isInInputElement = target.tagName === 'INPUT' ||
    ...
    for (const shortcut of shortcutsRef.current.values()) {
      if (matchesShortcut(event, shortcut)) {
        // 某些快捷键在输入框中也应该工作
        const allowInInput = shortcut.category === 'accessibility' ||
  ...
  const getShortcutsByCategory = useCallback((category: KeyboardShortcut['category']): KeyboardShortcut[] => {
    return getRegisteredShortcuts().filter(shortcut => shortcut.category === category);
...
export const useKeyboardHelp = () => {
  const { getShortcutsByCategory } = useKeyboardManager();
