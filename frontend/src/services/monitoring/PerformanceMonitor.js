...
class PerformanceMonitor {
    constructor() {
        ...
        this.thresholds = {
            memoryAccessTime: 10,
            indexedDBAccessTime: 100,
            fastgptAccessTime: 2000,
            errorRate: 0.05,
            cacheHitRate: 0.7,
            syncDuration: 30000,
            storageUsage: 0.8,
    ...
    recordAccess(_key, tier, duration, hit, dataSize) {
    ...
    recordSync(operation, duration, success, dataSize) {
        if (!this.isMonitoring) {
            return;
        }
        try {
    ...
    recordCacheEviction(key, reason, tier) {
        if (!this.isMonitoring) {
            return;
        }
        try {
    ...
    checkAccessThresholds(data) {
        const thresholds = this.getThresholdsForTier(data.tier);
        if (data.accessTime > thresholds.maxAccessTime) {
            this.createAlert('warning', 'slow_access', data.accessTime, thresholds.maxAccessTime, `${data.tier}访问时间过慢: ${data.accessTime.toFixed(2)}ms`, ['优化查询索引', '减少数据传输量', '检查网络连接']);
        }
        if (!data.success) {
            this.createAlert('error', 'access_failure', 1, 0, `${data.tier}访问失败`, ['检查存储提供者状态', '验证数据格式', '重试操作']);
        }
    }
    checkSyncThresholds(data) {
        if (data.duration > this.thresholds.syncDuration) {
            this.createAlert('warning', 'slow_sync', data.duration, this.thresholds.syncDuration, `同步操作过慢: ${data.duration}ms`, ['减少同步数据量', '优化网络请求', '检查远程服务状态']);
        }
        if (!data.success) {
            this.createAlert('error', 'sync_failure', 1, 0, `同步操作失败: ${data.operation}`, ['检查网络连接', '验证认证信息', '重试同步操作']);
        }
    }
    checkEvictionThresholds(_data) {
        const recentEvictions = this.evictionHistory.filter(e => e.timestamp > Date.now() - 60000);
        if (recentEvictions.length > 100) {
            this.createAlert('warning', 'excessive_eviction', recentEvictions.length, 50, `缓存清理过于频繁: ${recentEvictions.length}次/分钟`, ['增加缓存大小', '优化缓存策略', '减少数据大小']);
        }
    }
    getThresholdsForTier(tier) {
        switch (tier) {
            case hybrid_storage_1.StorageTier.MEMORY:
                return { maxAccessTime: this.thresholds.memoryAccessTime };
            case hybrid_storage_1.StorageTier.INDEXED_DB:
                return { maxAccessTime: this.thresholds.indexedDBAccessTime };
            case hybrid_storage_1.StorageTier.FASTGPT_REMOTE:
                return { maxAccessTime: this.thresholds.fastgptAccessTime };
            default:
                return { maxAccessTime: 1000 };
        }
    }
    createAlert(type, metric, value, threshold, message, suggestions) {
        const alert = {
            type,
            metric,
            value,
    ...
    async calculateStorageMetrics(since) {
        const recentAccess = this.accessHistory.filter(data => data.timestamp >= since);
        const memoryAccess = recentAccess.filter(data => data.tier === hybrid_storage_1.StorageTier.MEMORY);
        const indexedDBAccess = recentAccess.filter(data => data.tier === hybrid_storage_1.StorageTier.INDEXED_DB);
        const fastgptAccess = recentAccess.filter(data => data.tier === hybrid_storage_1.StorageTier.FASTGPT_REMOTE);
        return {
    ...
    calculateMemoryMetrics(accessData) {
    ...
    calculateIndexedDBMetrics(accessData) {
        const totalOps = accessData.length;
        const successfulOps = accessData.filter(data => data.success).length;
        const hitOps = accessData.filter(data => data.operation === 'read_hit').length;
        const averageTime = totalOps > 0
    ...
    calculateFastgptMetrics(accessData) {
        const totalOps = accessData.length;
        const successfulOps = accessData.filter(data => data.success).length;
        const hitOps = accessData.filter(data => data.operation === 'read_hit').length;
        const averageTime = totalOps > 0
    ...
    getActiveAlerts() {
        const alerts = [];
        const recentAccess = this.accessHistory.filter(data => data.timestamp > Date.now() - 300000);
    ...
    async generateOptimizationSuggestions(storage, sync, cache) {
        const suggestions = [];
