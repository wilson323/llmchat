...
exports.parseReasoningPayload = exports.normalizeReasoningDisplay = void 0;
;
const isReasoningData = (value) => {
...
const normalizeReasoningDisplay = (input) => {
    ...
    const shouldUseTitle = (matchedByPattern && (effectiveRest.length > 0 || inlineRemainder.length > 0 || restJoined.length > 0)) ||
...
exports.normalizeReasoningDisplay = normalizeReasoningDisplay;
...
const mergeTotalSteps = (current, next, fallback) => {
...
const parseReasoningPayload = (payload) => {
    ...
    while (queue.length > 0) {
        ...
        if (typeof item === 'object' && item !== null) {
            if (isReasoningData(item)) {
                ...
                const candidate = normalizeText(reasoningData.thought ??
                ...
                if (reasoningData.nextThoughtNeeded === false ||
                ...
                if (candidate) {
                    ...
                    const title = normalizeText(typeof obj.title === 'string' ? obj.title : undefined) ?? undefined;
                    const stepData = {
                        content: candidate,
                        raw: toJsonValue(item),
                    };
...
exports.parseReasoningPayload = parseReasoningPayload;
exports.default = exports.parseReasoningPayload;
